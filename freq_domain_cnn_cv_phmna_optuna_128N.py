...
print('############################################ freq domain started 128N #################################')
import pandas as pd
import numpy as np
import random
import gc
...
def build_model_cnn(input_shape):  # random search passes this hyperparameter() object 

    
    model = Sequential()
    model.add(keras.layers.Conv1D(filters=64, kernel_size=4, activation='relu', input_shape=input_shape))    
...
for j in range(6):
    ...
    # training
    model=build_model_cnn(x_tr[0].shape)
...
print('results freq domain 128N')
print(acc_val_all)
